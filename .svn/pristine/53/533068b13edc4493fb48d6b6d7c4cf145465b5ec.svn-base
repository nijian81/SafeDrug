...
/**
 * Created by lenovo on 2015/3/9. 药品查询页面，进入药品详情页面，点击选择更多之后，弹出的药品规格展示项
 */
public class CompanyItemDescAdapter extends BaseAdapter implements
		SectionIndexer {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		convertView = LayoutInflater.from(context).inflate(
				R.layout.company_item_desc, null);
		viewHolder.tvDesc = (TextView) convertView.findViewById(R.id.desc);
		viewHolder.tvRelativeLayout = (RelativeLayout) convertView
				.findViewById(R.id.relative);
		if (this.list.get(position).getFlag() == 1) {
			viewHolder.tvRelativeLayout.setBackgroundColor(Color.rgb(23, 107,
					92));
		}

...
public final class ViewfinderView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        // 得到一个位于屏幕正中间的正方形框，编程为屏幕宽度的3/5.
        Rect frame = cameraManager.getFramingRect();
        if (frame == null) {
            return;
        }
        // width和height得到是整个屏幕的宽度和长度
        int width = canvas.getWidth();
        int height = canvas.getHeight();

        // 顶部返回按钮
        // pic = BitmapFactory.decodeResource(getResources(), R.drawable.back);
        // canvas.drawBitmap(pic, width / 8, frame.top / 10, paint);
        // 画外部暗色的部分，
        // Draw the exterior (i.e. outside the framing rect) darkened
        paint.setColor(0xFF203A32);
        // 绘制扫描框上方的暗黑色阴影部分
        ...
        if (resultBitmap != null) {
            ...
            // R.drawable.open);
            // canvas.drawBitmap(pic, width * 4 / 9, frame.bottom + 200, paint);
            // paint.setColor(Color.WHITE);
            // paint.setTextSize(50);
            // canvas.drawText("打开", width * 37 / 72, frame.bottom + 430,
            // paint);
            // 在本类的最后添加了图片的点击监控事件。
